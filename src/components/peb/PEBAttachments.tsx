...
import { PEBAttachment, PEB_DOCUMENT_TYPES } from '@/types/peb';
...
interface PEBAttachmentsProps {
  attachments: AttachmentWithFile[];
  onAttachmentsChange: (attachments: AttachmentWithFile[]) => void;
  existingAttachments?: PEBAttachment[];
  isReadOnly?: boolean;
  transportMode?: string;
}

export function PEBAttachments({ 
  attachments, 
  onAttachmentsChange, 
  existingAttachments = [],
  isReadOnly,
  ...
  const getRelevantDocTypes = () => {
    if (!transportMode) return PEB_DOCUMENT_TYPES;

    // Filter out irrelevant document types
    if (transportMode === 'AIR') {
      return PEB_DOCUMENT_TYPES.filter(type => type.value !== 'BL');
    } else if (transportMode === 'SEA') {
      return PEB_DOCUMENT_TYPES.filter(type => type.value !== 'AWB');
    }

    return PEB_DOCUMENT_TYPES;
  };
    ...
    <div className="space-y-4">
      ...
      {attachments.length === 0 && existingAttachments.length === 0 && (
        <div className="border-2 border-dashed rounded-lg p-8 text-center text-muted-foreground">
          ...
          {!isReadOnly && (
            <p className="text-xs mt-1">Upload supporting documents like invoices, packing lists, etc.</p>
          )}
        </div>
      )}
      ...
      {!isReadOnly && (
        <div className={cn(
          ...
          <p className="font-medium mb-2 flex items-center gap-2">
            ...
            {transportMode 
              ? `Required Documents for PEB (${transportMode}):` 
              : 'Required Documents for PEB:'}
          </p>
