...
export function PIBItemsTable({
  ...
  const fetchHSCodes = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from("hs_codes")
        .select(
          "id, code, name, description, unit, bm_rate, ppn_rate, pph_rate",
        )
        .eq("is_active", true)
        .order("code");
  ...
  const handleChange = (field: keyof PIBItem, value: unknown) => {
    ...
    if (field === "hs_code_id") {
      ...
      if (hsCode) {
        ...
        // Auto-set packaging_code = "CR" for live animals (HS 01)
        if (hsCode.code.startsWith('01')) {
          updatedItem.packaging_code = "CR";
        }
      }
    }

    // Auto-sync quantity to package_count for live animals (HS code starts with 01)
    if (field === "quantity" && updatedItem.hs_code?.startsWith('01')) {
      updatedItem.package_count = value as number;
    }

    // Recalculate when relevant fields change
    if (
      ["quantity", "unit_price", "bm_rate", "ppn_rate", "pph_rate"].includes(
        field,
      )
    ) {
  ...
  const validate = (): boolean => {
    ...
    if (!currentItem.product_description)
      newErrors.product_description = "Description is required";
    if (!currentItem.quantity || currentItem.quantity <= 0)
      newErrors.quantity = "Quantity must be greater than 0";
    if (!currentItem.quantity_unit) {
      newErrors.quantity_unit =
        "Unit is required. HS Code must have a valid unit.";
    }
    ...
    if (
      currentItem.gross_weight &&
      currentItem.net_weight &&
      currentItem.gross_weight < currentItem.net_weight
    ) {
    ...
    if (!currentItem.packaging_code) {
      newErrors.packaging_code = "Package Type is required";
    }
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/30 border-b">
                <tr className="text-left">
                  <th className="p-2 px-3 font-medium text-muted-foreground text-xs w-12">
                    #
                  </th>
                  ...
                  {!isReadOnly && (
                    <th className="p-2 px-3 font-medium text-muted-foreground text-xs text-center w-20">
                      Actions
                    </th>
                  )}
                </tr>
              ...
              <tbody>
                {items.map((item, index) => (
                  <tr key={index} className="border-b last:border-0">
                    ...
                    <td className="p-2 text-right font-mono">
                      {item.cif_value?.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                      })}
                    </td>
                    <td className="p-2 text-right font-mono text-xs">
                      <div>
                        {item.bm_amount?.toLocaleString(undefined, {
                    ...
                    <td className="p-2 text-right font-mono text-xs">
                      <div>
                        {item.ppn_amount?.toLocaleString(undefined, {
                          maximumFractionDigits: 0,
                        })}
                      </div>
                      <div className="text-muted-foreground">
                    ...
                    <td className="p-2 text-right font-mono text-xs">
                      <div>
                        {item.pph_amount?.toLocaleString(undefined, {
                          maximumFractionDigits: 0,
                        })}
                      </div>
                      <div className="text-muted-foreground">
                    ...
                    <td className="p-2 text-right font-mono font-medium">
                      {item.total_tax?.toLocaleString(undefined, {
                        maximumFractionDigits: 0,
                      })}
                    </td>
                    {!isReadOnly && (
              ...
              <tfoot className="bg-muted/20 border-t">
                <tr>
                  <td
                    colSpan={4}
                    className="p-2 px-3 text-right font-medium text-sm"
                  >
                    Totals:
                  </td>
                  ...
                  <td
                    colSpan={3}
                    className="p-2 text-right font-mono text-xs text-muted-foreground"
                  >
                    IDR{" "}
                    {totalCIFIDR.toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </td>
          ...
          <div className="grid gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select
                  value={currentItem.hs_code_id || ""}
                  onValueChange={(v) => handleChange("hs_code_id", v)}
                >
                  <SelectTrigger className="h-8 text-sm">
                    <SelectValue
                      placeholder={
                        loadingHSCodes ? "Loading..." : "Select HS Code"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">
                  Unit Price (FOB) <span className="text-red-500">*</span>
                </Label>
                <Input
                  type="number"
                  step="0.01"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("net_weight", parseFloat(e.target.value) || 0)
                  }
                  className="h-8 text-sm"
                />
                {errors.net_weight && (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select
                  value={currentItem.packaging_code || ""}
                  onValueChange={(v) => handleChange("packaging_code", v)}
                >
                  <SelectTrigger className="h-8 text-sm">
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="CR">Crate (CR)</SelectItem>
                    ...
                    <SelectItem value="BDL">Bundle (BDL)</SelectItem>
                    <SelectItem value="ROL">Roll (ROL)</SelectItem>
                    <SelectItem value="DRM">Drum (DRM)</SelectItem>
                  </SelectContent>
                </Select>
                {errors.packaging_code && (
                  <p className="text-xs text-red-500 flex items-center gap-1">
                    <AlertCircle className="h-3 w-3" />
                    {errors.packaging_code}
                  </p>
              ...
              <div className="space-y-1.5">
                <Label className="text-xs flex items-center gap-1">
                  Package Count
                  {currentItem.hs_code?.startsWith('01') && (
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleChange("package_count", parseInt(e.target.value) || 0)
                  }
                  disabled={currentItem.hs_code?.startsWith('01')}
                  className={cn(
                    "h-8 text-sm",
                    currentItem.hs_code?.startsWith('01') && "bg-slate-100 cursor-not-allowed"
                  )}
                />
                {currentItem.hs_code?.startsWith('01') && (
                  <p className="text-[10px] text-muted-foreground">
                    Auto-synced from quantity for live animals
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("bm_rate", parseFloat(e.target.value) || 0)
                    }
                    className="h-8 text-sm"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("ppn_rate", parseFloat(e.target.value) || 0)
                    }
                    className="h-8 text-sm"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("pph_rate", parseFloat(e.target.value) || 0)
                    }
                    className="h-8 text-sm"
                  />
                </div>
              ...
              <div className="grid grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-mono font-medium">
                    {(currentItem.cif_value || 0).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-mono font-medium">
                    IDR{" "}
                    {(currentItem.bm_amount || 0).toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </div>
                </div>
                <div>
                  ...
                  <div className="font-mono font-medium">
                    IDR{" "}
                    {(
                      (currentItem.ppn_amount || 0) +
                      (currentItem.pph_amount || 0)
                ...
                <div>
                  ...
                  <div className="font-mono font-bold text-primary">
                    IDR{" "}
                    {(currentItem.total_tax || 0).toLocaleString(undefined, {
                      maximumFractionDigits: 0,
                    })}
                  </div>
                </div>
