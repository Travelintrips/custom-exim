...
function mapCEISALane(
  ceisaLane: "HIJAU" | "KUNING" | "MERAH" | null,
): PIBLane | null {
...
export async function fetchPIBFromCEISA(
  ...
  const apiKey = import.meta.env.CEISA_API_KEY;
  ...
  try {
    ...
    if (fullParams.dateFrom)
      queryParams.append("dateFrom", fullParams.dateFrom);
    ...
    if (fullParams.pageSize)
      queryParams.append("pageSize", fullParams.pageSize.toString());
